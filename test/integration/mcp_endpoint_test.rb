...
class McpEndpointTest < ActionDispatch::IntegrationTest
  ...
  test "POST tools/list with valid key returns search tool" do
    ...
    assert_equal %w[explore_okr find_contributor get_ar_aging get_document get_enterprise_health get_executive_dashboard get_okr_grid get_person_metrics get_project_burnup get_project_contributors get_project_cost_breakdown get_resourcing_projections get_studio_health list_documents list_open_admin_tasks list_overdue_invoices list_payable_bills list_project_trackers list_projects_at_risk list_sources search], tool_names.sort,
