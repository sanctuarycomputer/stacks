...
class Resourcing::RunnLeaveRefreshTest < ActiveSupport::TestCase
  test "run! mirrors each non-archived person's leave into runn_leaves" do
    runn = mock("runn")
    ...
    svc = Resourcing::RunnLeaveRefresh.new(runn: runn)
    count = svc.run!
    assert_equal 1, count
    row = RunnLeave.find_by(runn_person_id: 10)
  ...
  test "run! replaces a person's prior leave rows" do
    RunnLeave.create!(runn_person_id: 10, start_date: Date.new(2029, 1, 1), end_date: Date.new(2029, 1, 2), refreshed_at: Time.current)
    runn = mock("runn")
    ...
    svc = Resourcing::RunnLeaveRefresh.new(runn: runn)
    svc.run!
    assert_equal [Date.new(2030, 5, 10)], RunnLeave.where(runn_person_id: 10).pluck(:start_date)
  end

  test "run! isolates a per-person failure and still mirrors the rest" do
    RunnLeave.create!(runn_person_id: 11, start_date: Date.new(2029, 1, 1), end_date: Date.new(2029, 1, 2),
    ...
    assert RunnLeave.find_by(runn_person_id: 10).present?
    assert_equal Date.new(2029, 1, 1), RunnLeave.find_by(runn_person_id: 11).start_date
  end
end
