...
class Mcp::WeeklyShipBlockToolTest < ActiveSupport::TestCase
  def tracker!(name:, client:, **attrs)
    fc = ForecastClient.create!(forecast_id: rand(1_000_000..9_999_999), name: client, archived: false, updated_at: Time.current)
  ...
  setup do
    ProjectTracker.any_instance.stubs(:income).returns(1000.0)
    ProjectTracker.any_instance.stubs(:spend).returns(1500.0)
  ...
  test 'by trackers: one summed block, weeks_left, ongoing flag, newest ship across trackers' do
  ...
  test 'by client: every open tracker for the client, completed ones excluded' do
