module Mcp
  class GetResourcingWorldTool < MCP::Tool
    tool_name 'get_resourcing_world'
    ...
    def self.call(window_days: 90, include_archived: false, server_context:)
      ...
      safe_read = lambda do |default, &blk|
        ...
        Rails.logger.warn("[get_resourcing_world] degraded read: #{e.class}")
      ...
      leave = RunnLeaveMirror.all.map do |lm|
      ...
      Rails.logger.warn("[Mcp::GetResourcingWorldTool] #{e.class}: #{e.message}")
      Sentry.capture_exception(e) if defined?(Sentry)
      # never echo upstream/internal error bodies to the caller
      Responses.error("get_resourcing_world failed; the error was logged")
    end
    ...
    def self.recent_actuals(trackers_by_runn_id, raw_people, raw_assignments, today)
      ...
      Rails.logger.warn("[Mcp::GetResourcingWorldTool] recent_actuals failed: #{e.class}: #{e.message}")
      Sentry.capture_exception(e) if defined?(Sentry)
      {}
    end
