...
```ruby
  ...
  test "an unlinked case-fold duplicate drives no Ghost write at all" do
    ...
    # The duplicate wants an EXTRA label the owner does not, so its label diff is still
    # non-empty after the owner's write has converged the member. Without that the
    # duplicate's diff is already a no-op and the test cannot see the guard at all.
  ...
  test "the pre-write recheck does not re-count what the decision phase already counted" do
    ...
    # decision phase), one a real candidate. The recheck then hits the ledger fast path
    # for the first. With count: true that increments already_handled a second time,
    # inflating the numbers the rollout review reads.
    enable_sources("xxix:", "index:shopify_customer")
    sys!(ghost_newsletter_prefix_map: { "xxix" => "nl-xxix", "index" => "nl-index" },
         ghost_newsletter_grants_enabled: "1")
    Contact.create!(email: "recount@example.com",
      sources: ["xxix:", "index:shopify_customer"], ghost_id: "m89")
    subscribed = { "newsletters" => [{ "id" => "nl-index", "name" => "Index", "status" => "active" }] }
    m = member(id: "m89", email: "recount@example.com", labels: %w[xxix: index:shopify_customer],
      extra: subscribed)
    ...
    ghost.expects(:all_members).returns([m])
    ghost.stubs(:newsletter_events_for).returns([])
    ghost.expects(:find_member).with("m89").returns(m)
    ghost.expects(:update_member).returns(
      m.merge("newsletters" => [{ "id" => "nl-index" }, { "id" => "nl-xxix" }]))
...
```ruby
    ...
    if contact.ghost_id.nil? &&
      ...
      # Counts CONTACTS skipped, not grants: this fires before the decision runs, so it
      # includes duplicates that had nothing to grant anyway.
