...
| Opt-in state (newsletter subscriptions) | **Shared** | **Amended 2026-09-16.** Stacks grants a newsletter at most once per member, derived from the first colon-segment of the contact's enabled sources. Ghost remains authoritative for revocation: stacks never unsubscribes anyone, and never re-subscribes anyone who has ever been subscribed before. See `docs/superpowers/specs/2026-09-16-ghost-source-newsletter-subscriptions-design.md`. |
...
**Enabled sources** live on the `System` singleton's Storext settings store as `ghost_synced_sources` (array of source strings, default `[]`) — matching where this app keeps other app-wide config (no dedicated table). Written by the admin checkbox UI. Note: `System.instance` memoizes in a class variable that is never invalidated. An earlier version of this note claimed only the checkbox *display* could go stale, "since the sweep runs in fresh rake dynos and always reads current state". That is **wrong**: the admin "Sync Now" button runs the whole sweep inline in a web dyno (`app/admin/ghost_sync.rb`). Worse, Storext writes the entire `settings` jsonb column, so a save made from a memoized instance silently reverts every other setting the row has gained since that instance was cached. Everything touching these settings must read a fresh `System.first_or_create!(settings: {})`.
